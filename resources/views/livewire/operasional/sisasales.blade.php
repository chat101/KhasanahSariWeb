    <div>
        ...
        <div data-kontribusi-tabs x-data="{
            tab: 'target',
            h: 0,
            syncH() { this.$nextTick(() => this.h = this.$refs.panel?.scrollHeight || 0) }
        }" x-init="syncH()" class="space-y-4">
                                ...
                                <table class="min-w-[1200px] w-full text-xs text-left">
                                    ...
                                    <tbody class="divide-y divide-gray-100">
                                        @forelse(($rowsTarget ?? []) as $r)
                                            @php
                                                $sp = is_null($r['selisih_persen'] ?? null)
                                                    ? null
                                                    : (float) $r['selisih_persen'];
                                            ...
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td
                                                    class="px-3 py-2 border-r text-center font-semibold {{ $clsP }}">
                                                    @if (is_null($sp))
                                                        -
                                                    @else
                                                        {{ number_format($sp, 2, ',', '.') }}
                                                ...
                                                <td
                                                    class="px-3 py-2 border-r text-right font-semibold {{ $clsR }}">
                                                    {{ $sr === 0 ? '-' : number_format($sr, 0, ',', '.') }}
                                                </td>

                                                <td
                                                    class="px-3 py-2 border-r text-right font-semibold {{ $clsK }}">
                                                ...
                                                <td
                                                    class="px-3 py-2 border-r text-right font-semibold {{ $clsV }}">
                                                    {{ $v === 0 ? '-' : number_format($v, 0, ',', '.') }}
                                                </td>
                                                <td class="px-3 py-2 border-r text-center">%retur</td>
                                                {{-- <td class="px-3 py-2 border-r text-center">{{ $r['retur_persen'] ?? '-' }}</td> --}}
                                                <td class="px-3 py-2 border-r text-right">
                                                    {{ number_format($r['retur_rp'] ?? 0, 0, ',', '.') }}</td>

                                                {{-- <td class="px-3 py-2 border-r text-right">retur Rp</td> --}}

                                                {{-- GAS --}}
                                                @php
                                                    $gasP = is_null($r['gas_persen'] ?? null)
                                                        ? null
                                                        : (float) $r['gas_persen'];
                                                    $telP = is_null($r['telur_persen'] ?? null)
                                                        ? null
                                                        : (float) $r['telur_persen'];
                                                ...
                                                <td
                                                    class="px-3 py-2 border-r text-center font-semibold {{ $clsGasP }}">
                                                    {{ is_null($gasP) ? '-' : number_format($gasP, 2, ',', '.') }}
                                                </td>

                                                <td
                                                    class="px-3 py-2 border-r text-right font-semibold {{ $clsGas }}">
                                                    {{ $gasR === 0 ? '-' : number_format($gasR, 0, ',', '.') }}
                                                </td>

                                                {{-- TELUR --}}
                                                <td
                                                    class="px-3 py-2 border-r text-center font-semibold {{ $clsTelP }}">
                                                    {{ is_null($telP) ? '-' : number_format($telP, 2, ',', '.') }}
                                                </td>

                                                <td
                                                    class="px-3 py-2 border-r text-right font-semibold {{ $clsTel }}">
...
@php
    $clsFoot =
        $totTotalKontribusi < 0 ? 'bg-rose-50 text-rose-700'
      : ($totTotalKontribusi > 0 ? 'bg-emerald-50 text-emerald-700'
