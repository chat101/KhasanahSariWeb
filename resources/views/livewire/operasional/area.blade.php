<div>
    <div class="p-2 space-y-4 bg-gray-100 text-gray-800 text-sm rounded-lg shadow-lg">
            ...
            <div class="bg-white rounded-lg shadow border border-gray-200 p-4 space-y-0 text-black">
                <div class="flex items-center justify-between gap-2">
                    <h2 class="text-sm font-semibold flex items-center gap-2">
                        <span class="inline-flex h-7 w-7 items-center justify-center rounded-full bg-indigo-100 text-indigo-600 text-xs">AR</span>
                        Master Area
                    </h2>
                    <span class="text-[11px] text-gray-500">Kelola data area</span>
                </div>
            </div>
          </div>
        ...
        {{-- Search Box + Filter - Directly Above Table --}}
        <div class="flex items-center gap-2 mb-2">
            <div class="relative w-full md:w-1/3">
                <input wire:model.live="search" class="py-1 pl-8 pr-8 border border-gray-300 rounded-lg focus:ring-1 focus:ring-blue-300 bg-white text-xs w-full" type="search" placeholder="Cari area / wilayah..." />
            ...
            <select wire:model.live="filterWilayah" class="py-1 px-3 border border-gray-300 rounded-lg bg-white text-xs">
          ...
          <table class="min-w-full text-xs">
                <thead class="bg-white text-gray-700 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-semibold">Wilayah</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($areas as $a)
                        <tr class="transform transition-all duration-150 hover:-translate-y-0.5 hover:shadow-sm">
                            <td class="px-4 py-3">{{ ($areas->currentPage()-1)*$areas->perPage() + $loop->iteration }}</td>
                            <td class="px-4 py-3">{{ $a->id }}</td>
                            <td class="px-4 py-3">{{ $a->wilayah?->nama_wilayah ?? '-' }}</td>
                        ...
                        <tr>
                            <td colspan="6" class="text-center py-12">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-sm text-gray-500 font-medium">Tidak ada data area</p>
                                    <p class="text-xs text-gray-400 mt-1">Mulai tambahkan area baru untuk melihat daftar di sini</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
