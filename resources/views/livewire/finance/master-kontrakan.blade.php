<div>
    <div x-show="activeTab === 'kontrakan'">
            ...
            <table class="min-w-full text-left text-xs text-gray-300">
                <thead class="bg-gray-700 text-gray-200 uppercase">
                    <tr>
                        ...
                        <th class="px-3 py-2">Harga Sewa</th>
                ...
                <tbody class="divide-y divide-gray-700">
                    @php $no = ($tokos->currentPage()-1) * $tokos->perPage(); @endphp

                    @forelse($tokos as $toko)
                        @forelse($toko->kontrakans as $k)
                            @php $no++; @endphp
                            <tr>
                                <td class="px-3 py-2">{{ $no }}</td>
                                <td class="px-3 py-2">{{ $toko->nmtoko }}</td>
                                ...
                                <td class="px-3 py-2">{{ number_format($k->nilai_sewa ?? 0, 0, ',', '.') }}</td>
                        ...
                        @empty
                            ...
                            <tr>
                                <td class="px-3 py-2">{{ $no }}</td>
                                <td class="px-3 py-2">{{ $toko->nmtoko }}</td>
                                <td class="px-3 py-2 text-center text-gray-400" colspan="7">
                                    Tidak ada kontrakan untuk {{ $toko->nmtoko }}
                                </td>
                                <td class="px-3 py-2">
                                    <button class="px-3 py-2 text-xs bg-blue-600 hover:bg-blue-700 text-white rounded"
                                        wire:click="openEditKontrakan(null, {{ $toko->id }})">+ Tambah</button>
                                </td>
                            </tr>
                        @endforelse
                    @empty
