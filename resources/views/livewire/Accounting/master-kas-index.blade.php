<div>
    <div class="space-y-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
                <h2 class="text-lg font-semibold">Master Akun Kas / Bank</h2>
                <p class="text-xs text-gray-500">
                    Pengaturan akun kas dan bank (COA dengan penanda kas) untuk transaksi dan jurnal otomatis.
                </p>
            </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-100 dark:bg-zinc-700 text-gray-700 dark:text-gray-100">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-center w-28">Saldo Normal</th>
                    ...
                    @forelse($items as $index => $row)
                        <tr>
                            ...
                            <td class="px-3 py-2 text-right">
                                ...
                                <button type="button"
                                        onclick="if(!confirm('Yakin hapus akun kas ini?')) return false;"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-3 py-4 text-center text-gray-500">
                                Belum ada akun kas yang terdaftar.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <div class="fixed inset-0 z-40 flex items-center justify-center bg-black bg-opacity-40">
                <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-lg w-full max-w-md p-4">
                    <h3 class="text-base font-semibold mb-3">
                        {{ $editId ? 'Edit Akun Kas' : 'Tambah Akun Kas' }}
                    </h3>
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-gray-400 mt-1">
                                Umumnya akun kas/bank memiliki saldo normal di sisi <strong>Debet (D)</strong>.
                            </p>
                        </div>
                        <div>
                            <label class="block text-gray-600 mb-1">Role (opsional)</label>
                            <select wire:model="default_role"
                                    class="w-full border rounded-lg px-3 py-2 text-sm bg-white text-black">
                                <option value="">- Tidak ada -</option>
                                <option value="kas_kecil">kas_kecil</option>
                                <option value="kas_bank">kas_bank</option>
                                {{-- bisa tambah role lain nanti --}}
                            </select>
                            @error('default_role')
                            <div class="text-xs text-red-600 mt-1">{{ $message }}</div>
                            @enderror
                            <p class="text-[11px] text-gray-400 mt-1">
                                Role ini dipakai untuk jurnal otomatis. Misal:
                                <strong>kas_kecil</strong> untuk akun kas kecil,
                                <strong>kas_bank</strong> untuk akun kas bank utama.
                            </p>
                        </div>
                    </div>
