<div>
    <div x-data="{ activeTab: 'poprok' }" class="w-full">
        ...
        <div class="px-1 py-1 border-b border-gray-200 dark:border-zinc-700 flex items-center justify-between gap-3">
            <div class="flex items-center gap-2">
                ...
                <h2 class="text-base font-semibold">Hasil Poprok</h2>
            </div>
        ...
        <div class="px-4 pt-3 flex items-end justify-between gap-3">
            ...
            <div class="mt-4 flex items-center justify-end gap-2">
                ...
                <button type="button" wire:click="submit" wire:loading.attr="disabled" wire:target="submit"
                    class="px-4 py-2 rounded bg-emerald-600 hover:bg-emerald-700 text-white text-sm shadow">

                    <span wire:loading.remove wire:target="submit">Submit Realisasi</span>
            ...
            <table id="grid-body" class="min-w-full mt-1.5 text-[12px] text-left text-gray-800 dark:text-zinc-100">
                <thead class="bg-gray-50 dark:bg-zinc-900 text-gray-600 dark:text-gray-300 sticky top-0 z-10">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-center border-b border-gray-200 dark:border-zinc-700">Nama Barang</th>
                        <th class="px-3 py-2 text-center border-b border-gray-200 dark:border-zinc-700">Produksi (tong)
                        </th>
                        <th class="px-3 py-2 text-center border-b border-gray-200 dark:border-zinc-700">Target Giling
                        </th>
                        <th class="px-3 py-2 text-center border-b border-gray-200 dark:border-zinc-700">Realisasi Giling
                        </th>
                        <th class="px-3 py-2 text-center border-b border-gray-200 dark:border-zinc-700">Selisih</th>
                    </tr>
                </thead>

                <tbody>
                    @forelse($perintahproduksi as $index => $produk)
                        <tr wire:key="row-{{ $perintah_id }}-{{ $produk['mproducts_id'] }}">
                            ...
                            <td class="td text-center">
                                <input type="text"
                                    wire:key="input-{{ $perintah_id }}-{{ $produk['mproducts_id'] }}" placeholder="0"
                                    class="w-24 px-2 py-1 border border-gray-300 rounded bg-white text-gray-900
                                      focus:ring focus:ring-indigo-200 focus:border-indigo-400"
                                    x-data="numberMask({
                                        initial: @js($realisasi[$index] ?? null),
                                        setWire: (v) => $wire.set('realisasi.{{ $index }}', v)
                                    })" x-init="// paksa set tampilan dari initial tiap mount
                                    display = (initial !== null && initial !== '') ? nf.format(parseInt(initial)) : ''" x-model="display"
                                    x-on:input="onInput($event)" x-on:paste="onPaste($event)"
                                    @keydown.enter.prevent="
                                    const inputs=[...$el.closest('table').querySelectorAll('input[type=text]')];
                                    const i=inputs.indexOf($el); if(i>=0 && i<inputs.length-1) inputs[i+1].focus();
                                    " />
                            </td>

                            @php
                                $selisih = intval($realisasi[$index] ?? 0) - $produk['sisa_target'];
                            @endphp
                            <td class="td text-center {{ $selisih < 0 ? 'text-red-600 font-semibold' : '' }}">
                                {{ number_format($selisih, 0, ',', '.') }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td class="td text-center text-gray-300 dark:text-gray-400" colspan="6">
                                Tidak ada data
                            </td>
                        </tr>
                    @endforelse
                ...
                @if (count($perintahproduksi) > 0)
                    @php
                        ...
                        $totalSelisih = $totalRealisasi - $totalSisaTarget;
                    @endphp
                    <tfoot>
                        <tr class="bg-gray-100 dark:bg-zinc-800 font-semibold">
                            <td colspan="2"
                                class="px-3 py-2 text-right border-t border-gray-200 dark:border-zinc-700">
                                Total
                            </td>
                            ...
                            <td
                                class="text-center border-t border-gray-200 dark:border-zinc-700 {{ $totalSelisih < 0 ? 'text-red-600 font-semibold' : '' }}">
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('numberMask', ({
            ...
            init() {
                if (initial !== null && initial !== '') this.display = this.nf.format(parseInt(
                    initial));
            },
            toRaw(str) {
                // ambil hanya digit
            ...
            format(raw) {
                return (raw === null) ? '' : this.nf.format(raw);
            },
            onInput(e) {
                // simpan posisi kursor dari kanan
                const prev = e.target.value;
                const fromRight = prev.length - e.target.selectionStart;

                const raw = this.toRaw(e.target.value);
                this.display = this.format(raw);

                // kembalikan kursor kira-kira di posisi semula
                const el = e.target;
                const newPos = Math.max(0, this.display.length - Math.min(fromRight, this.display
                    .length));
                requestAnimationFrame(() => {
                    el.setSelectionRange(newPos, newPos);
                });

                // sinkron ke Livewire (null kalau kosong)
                setWire(raw);
            },
            onPaste(e) {
                e.preventDefault();
                const text = (e.clipboardData || window.clipboardData).getData('text');
                const raw = this.toRaw(text);
                this.display = this.format(raw);
                setWire(raw);
            }
        }));
