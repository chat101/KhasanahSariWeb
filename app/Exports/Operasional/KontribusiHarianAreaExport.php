...
{
    public function __construct(public array $rows, public array $grandTotals, public string $start, public string $end) {}
    ...
    public function array(): array
    {
        ...
        foreach ($this->rows as $outlet => $byTanggal) {
            ...
            $totalToko = [
                'target' => ['hrg' => 0, 'selisih_rp' => 0, 'kontribusi' => 0, 'disc' => 0, 'retur' => 0, 'gas' => 0, 'telur' => 0, 'loss' => 0, 'kurang_setoran' => 0, 'total' => 0],
                'bl' => ['hrg' => 0, 'selisih_rp' => 0, 'kontribusi' => 0, 'disc' => 0, 'retur' => 0, 'gas' => 0, 'telur' => 0, 'loss' => 0, 'kurang_setoran' => 0, 'total' => 0],
            ];
            ...
            foreach ($byTanggal as $tgl => $list) {
                foreach ($list as $r) {
                    ...
                    $totalToko[$bucket]['kurang_setoran'] += $this->toInt($r['kurang_setoran'] ?? 0);
            ...
            foreach ($byTanggal as $tgl => $list) {
                foreach ($list as $r) {
                    ...
                    $out[] = [$outlet, (string) $tgl, (string) ($r['type'] ?? ''), $selPctDec, $this->toInt($r['selisih_rp'] ?? 0), $this->toInt($r['kontribusi'] ?? 0), $discPctDec, $this->toInt($r['disc_rp'] ?? 0), $retPctDec, $this->toInt($r['retur_rp'] ?? 0), $gasPctDec, $this->toInt($r['gas_rp'] ?? 0), $telPctDec, $this->toInt($r['telur_rp'] ?? 0), $this->toInt($r['loss_bahan'] ?? 0), $this->toInt($r['kurang_setoran'] ?? 0), $this->toInt($r['total_kontribusi'] ?? 0)];
                }
            }
            ...
            foreach (['target' => 'BY TARGET', 'bl' => 'BY BULAN LALU'] as $bucket => $label) {
                ...
                $out[] = [$outlet, '', 'TOTAL TOKO ' . $label, $this->pctSelisihDec($totalToko[$bucket]['selisih_rp'], $hrg), (int) $totalToko[$bucket]['selisih_rp'], (int) $totalToko[$bucket]['kontribusi'], $this->pctDec($totalToko[$bucket]['disc'], $hrg), (int) $totalToko[$bucket]['disc'], $this->pctDec($totalToko[$bucket]['retur'], $hrg), (int) $totalToko[$bucket]['retur'], $this->pctDec($totalToko[$bucket]['gas'], $hrg), (int) $totalToko[$bucket]['gas'], $this->pctDec($totalToko[$bucket]['telur'], $hrg), (int) $totalToko[$bucket]['telur'], (int) $totalToko[$bucket]['loss'], (int) $totalToko[$bucket]['kurang_setoran'], (int) $totalToko[$bucket]['total']];
            }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->mergeCells('P1:P2'); // KURANG SETORAN
                $sheet->mergeCells('Q1:Q2'); // TOTAL
                ...
                foreach (['D', 'G', 'I', 'K', 'M'] as $col) {
                    $sheet
                        ->getStyle("{$col}3:{$col}{$highestRow}")
                        ->getNumberFormat()
                        ->setFormatCode('0.00%');
                }

                // Rp columns: E,F,H,J,L,N,O,P,Q
                foreach (['E', 'F', 'H', 'J', 'L', 'N', 'O', 'P', 'Q'] as $col) {
                    $sheet
                        ->getStyle("{$col}3:{$col}{$highestRow}")
                        ->getNumberFormat()
                        ->setFormatCode('#,##0');
                ...
                $sheet
                    ->getStyle("A3:C{$highestRow}")
                    ->getAlignment()
                    ->setHorizontal(Alignment::HORIZONTAL_LEFT);
                $sheet
                    ->getStyle("D3:Q{$highestRow}")
                    ->getAlignment()
                    ->setHorizontal(Alignment::HORIZONTAL_RIGHT);
                $sheet
                    ->getStyle("A3:Q{$highestRow}")
                    ->getAlignment()
                ...
                $numericCols = [
                    'D',
                    'E',
                    'F', // Selisih % | Selisih Rp | Kontribusi
                    'G',
                    'H', // Disc % | Disc Rp
                    'I',
                    'J', // Retur % | Retur Rp
                    ...
                    'O',
                    'P',
                    'Q', // Loss | Kurang Setoran | Total Kontribusi
                ];
